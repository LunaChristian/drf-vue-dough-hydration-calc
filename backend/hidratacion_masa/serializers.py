...
class HidratacionMasaSerializer(serializers.Serializer):
    ...
    cantidad_agua = serializers.DecimalField(
        ...
        decimal_places=2,
        min_value=0.1,
        help_text="Cantidad de agua en gramos"
    )
    ...
    def validate(self, data):
        ...
        if agua > harina:
            raise serializers.ValidationError({
                'cantidad_agua': 'El agua no puede ser mayor que la cantidad de harina'
            })
        ...
        if hidratacion < 50:
            raise serializers.ValidationError({
                'cantidad_agua': f'Hidratación muy baja ({hidratacion:.1f}%). Mínimo recomendado: 50%'
            })
        
        # Regla de negocio 3: Hidratación máxima del 80%
        if hidratacion > 80:
            raise serializers.ValidationError({
                'cantidad_agua': f'Hidratación muy alta ({hidratacion:.1f}%). Máximo recomendado: 80%'
            })
